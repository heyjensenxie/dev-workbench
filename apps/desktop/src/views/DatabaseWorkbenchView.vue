...
import { Check, ChevronDown, ChevronRight, CircleAlert, Clipboard, Code2, Columns3, Copy, Database, FileCode2, FileDown, FileJson, History, KeyRound, Link2, List, LoaderCircle, MoreHorizontal, Play, Plus, RefreshCw, Search, Server, ShieldAlert, Table2, Terminal, Trash2, Unplug, WandSparkles, WrapText, X, Zap } from 'lucide-vue-next'
...
import { queryResultToCsv, queryResultToJson, type DatabaseExportFormat } from '../utils/databaseExport'
...
const exportingResult = ref<DatabaseExportFormat>()
const search = ref('')
...
const currentColumns = computed(() => activeConnection.value && activeDatabase.value && activeTable.value ? loadedColumns.value[tableKey(activeConnection.value.id, activeDatabase.value, activeTable.value)] ?? [] : [])
const currentIndexes = computed(() => activeConnection.value && activeDatabase.value && activeTable.value ? loadedIndexes.value[tableKey(activeConnection.value.id, activeDatabase.value, activeTable.value)] ?? [] : [])
...
function quoteMysqlIdentifier(identifier: string): string { return `\`${identifier.replaceAll('`', '``')}\`` }
function tableKey(connectionId: string, databaseName: string, tableName: string): string { return `${connectionId}:${databaseName}:${tableName}` }
...
async function selectTable(connection: DatabaseConnection, table: TableInfo, databaseName = activeDatabase.value): Promise<void> {
  if (!databaseName) return
  const key = tableKey(connection.id, databaseName, table.name)
  activeConnectionId.value = connection.id; activeDatabase.value = databaseName; activeTable.value = table.name; workbench.context.setDatabase({ connectionId: connection.id, type: connection.type, database: databaseName, selectedTable: table.name, activeQuery: query.value })
  ...
  loadedColumns.value[key] ??= await session.getTableColumns({ database: databaseName, table: table.name })
  loadedIndexes.value[key] ??= await session.getIndexes({ database: databaseName, table: table.name })
}
async function toggleTable(connection: DatabaseConnection, table: TableInfo, databaseName: string): Promise<void> { const key = tableKey(connection.id, databaseName, table.name); if (expandedTables.value.has(key)) { expandedTables.value.delete(key); return } await selectTable(connection, table, databaseName) }
function generateSelect(table = activeTable.value): void { if (!table) return; const qualifiedTable = activeDatabase.value ? `${quoteMysqlIdentifier(activeDatabase.value)}.${quoteMysqlIdentifier(table)}` : quoteMysqlIdentifier(table); query.value = `SELECT *\nFROM ${qualifiedTable}\nLIMIT 100;`; resultView.value = 'result'; flash(t('dbToastSelectGenerated')) }
async function openTable(connection: DatabaseConnection, table: TableInfo, databaseName: string): Promise<void> { await selectTable(connection, table, databaseName); generateSelect(table.name); await nextTick(); await runQuery() }
async function openStructure(connection: DatabaseConnection, table: TableInfo, databaseName: string): Promise<void> { activeConnectionId.value = connection.id; activeDatabase.value = databaseName; activeTable.value = table.name; resultView.value = 'structure'; await selectTable(connection, table, databaseName) }
async function showDdl(connection: DatabaseConnection, table: TableInfo, databaseName: string): Promise<void> { activeConnectionId.value = connection.id; activeDatabase.value = databaseName; activeTable.value = table.name; const session = sessions.get(connection.id); if (session) ddlByTable.value[tableKey(connection.id, databaseName, table.name)] = await session.getCreateTable({ database: databaseName, table: table.name }); resultView.value = 'ddl' }
...
async function exportResult(format: DatabaseExportFormat): Promise<void> {
  const currentResult = result.value
  if (!currentResult) { flash(t('dbNoResultToExport')); return }
  exportingResult.value = format
  try {
    const extension = format === 'csv' ? 'csv' : 'json'
    const path = await save({
      title: format === 'csv' ? t('dbExportCsvDialogTitle') : t('dbExportJsonDialogTitle'),
      defaultPath: `query-result-${new Date().toISOString().replaceAll(':', '-')}.${extension}`,
      filters: [{ name: format === 'csv' ? 'CSV' : 'JSON', extensions: [extension] }],
    })
    if (typeof path !== 'string') return
    const content = format === 'csv' ? queryResultToCsv(currentResult) : queryResultToJson(currentResult)
    await nativeBridge.writeDatabaseExport(path, content)
    flash(format === 'csv' ? t('dbToastCsvExported') : t('dbToastJsonExported'))
  } catch (error) {
    const details = errorDetails(error, t('dbExportFailed'))
    showError(details.code, details.message)
  } finally { exportingResult.value = undefined }
}
    ...
    <div class="database-layout">
              ...
              <template v-for="database in databases[connection.id] ?? []" :key="database.name">
                ...
                <div v-if="expandedDatabases.has(`${connection.id}:${database.name}`)" class="tree-children nested">
                  ...
                  <template v-for="table in visibleTablesFor(connection.id, database.name)" :key="table.name">
                    <button v-if="table.type === 'table'" class="tree-row table-row" :class="{ active: activeTable === table.name && activeDatabase === database.name && activeConnectionId === connection.id }" @dblclick.stop="openTable(connection, table, database.name)" @click="selectTable(connection, table, database.name)"><ChevronDown v-if="expandedTables.has(tableKey(connection.id, database.name, table.name))" :size="11" /><ChevronRight v-else :size="11" /><Table2 :size="12" /><span>{{ table.name }}</span><small v-if="table.rows !== undefined">{{ table.rows.toLocaleString() }}</small></button>
                    <div v-if="expandedTables.has(tableKey(connection.id, database.name, table.name))" class="table-subtree"><button class="tree-row sub-row" @click="openStructure(connection, table, database.name)"><Columns3 :size="11" /> {{ t('dbColumns') }} <small>{{ loadedColumns[tableKey(connection.id, database.name, table.name)]?.length ?? 0 }}</small></button><button class="tree-row sub-row" @click="showDdl(connection, table, database.name)"><Code2 :size="11" /> {{ t('dbDdl') }}</button></div>
                  </template>
                  ...
                  <button v-for="view in visibleTablesFor(connection.id, database.name).filter((item) => item.type === 'view')" :key="view.name" class="tree-row table-row view-row" :class="{ active: activeTable === view.name && activeDatabase === database.name && activeConnectionId === connection.id }" @dblclick.stop="openTable(connection, view, database.name)" @click="activeConnectionId = connection.id; activeDatabase = database.name; activeTable = view.name"><ChevronRight :size="11" /><Table2 :size="12" /><span>{{ view.name }}</span></button>
      ...
      <main class="database-main">
        ...
        <section class="result-panel">
          ...
          <div v-else-if="resultView === 'ddl'" class="ddl-view"><div class="ddl-heading"><span>{{ t('dbShowCreateTable', { table: activeTable ?? '' }) }}</span><button class="ghost-button" @click="copyText(activeConnectionId && activeDatabase && activeTable ? ddlByTable[tableKey(activeConnectionId, activeDatabase, activeTable)] ?? '' : '', t('dbToastDdlCopied'))"><Copy :size="13" /> {{ t('dbCopyDdl') }}</button></div><pre>{{ activeConnectionId && activeDatabase && activeTable ? ddlByTable[tableKey(activeConnectionId, activeDatabase, activeTable)] ?? t('dbLoadingDdl') : t('dbLoadingDdl') }}</pre></div>
...
.result-content { min-width: 0; overflow-x: hidden; }
.result-panel { min-width: 0; }
.grid-frame { min-width: 0; }
.grid-scroll { display: block; width: 100%; min-width: 0; min-height: 0; flex: 1 1 auto; overflow-x: auto; overflow-y: auto; overscroll-behavior: contain; overscroll-behavior-x: contain; scrollbar-gutter: stable; }
.data-grid { min-width: max-content; }
.data-grid th:not(.row-number):not(.grid-spacer), .data-grid td:not(.row-number):not(.row-actions) { min-width: 120px; }
.data-grid .grid-spacer, .data-grid .row-actions { width: 34px; min-width: 34px; }
</style>
